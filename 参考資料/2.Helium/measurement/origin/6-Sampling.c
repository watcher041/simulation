...
// 期待値を求める関数
  void Sampling(path *ps)
 {
    ...
    double erg,kin,pot,spec,dE,tp,up,fp,es,ed,pot_plus,value,ave;
    FILE *fd,*fs;
     ...
     for(step=1;step<=HEAT_MAX;step++){
      ...
      for(step=1;step<=MCS_MAX;step++){
        ...
        for(n=0;n<3*M;n++){
          ...
          up+=SU(ps,&n,&dammy,&dammy);
          fp+=SF(ps,&n,&dammy,&dammy);
        }
        es=-tp+up+3.0*fp;
      ...
      printf(" %2d E=%f [K] K=%f [K] U=%f [K] C=%f Ring=%f\n",samp,erg,kin,pot,spec,ave);
      fd=fopen(dd,"a");
      fprintf(fd,"%f %f %f %f\n",1.0/(TAU*(double)M),erg,spec,ave);
      fclose(fd);
