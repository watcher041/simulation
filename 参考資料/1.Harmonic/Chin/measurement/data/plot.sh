...
# 平均と理論値をエラーバーも兼ねて gnuplot で画面に出力 #
 gnuplot -e "
   ...
   set xlabel '~T{.8\~} [k_{B}T/ħω]';
   set ylabel '~E{.8\~} [E/ħω]';
   plot 0.5/tanh(0.5/x) title '理論値';
   replot 'plot.dat' using 1:2:3 with errorbars title '計算値' pt 7 pointsize 1;
   pause -1;
   set ylabel '~C{.8\~} [C/k_{B}]';
   plot 1.0/( x*x*4.0*sinh(0.5/x)*sinh(0.5/x) ) title '理論値';
   replot 'plot.dat' using 1:4:5 with errorbars title '計算値' pt 7 pointsize 1;
   pause -1;
 "
